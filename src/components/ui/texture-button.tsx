...
const buttonVariantsOuter = cva(
  'inline-flex items-center justify-center font-medium transition duration-300 ease-in-out cursor-pointer select-none disabled:cursor-not-allowed disabled:opacity-50 disabled:pointer-events-none',
  {
    variants: {
      variant: {
        primary:
          'w-full border-[1px] border-accent-600/30 dark:border-black/40 bg-gradient-to-b from-white/30 via-accent-400/20 to-black/30 dark:from-white/20 dark:via-accent-400/10 dark:to-black/50 p-[1px] transition duration-300 ease-in-out',
        accent:
          'w-full border-[1px] border-accent-600/30 dark:border-black/40 bg-gradient-to-b from-white/30 via-accent-400/20 to-black/30 dark:from-white/20 dark:via-accent-400/10 dark:to-black/50 p-[1px] transition duration-300 ease-in-out',
        destructive:
...
const innerDivVariants = cva('w-full h-full flex items-center justify-center font-medium', {
  variants: {
    variant: {
      primary:
        'gap-2 bg-gradient-to-b from-accent-500 to-accent-600 text-sm font-semibold text-on-accent transition duration-300 ease-in-out hover:from-accent-400 hover:to-accent-500 active:from-accent-600 active:to-accent-700',
      accent:
        'gap-2 bg-gradient-to-b from-accent-500 to-accent-600 text-sm font-semibold text-on-accent transition duration-300 ease-in-out hover:from-accent-400 hover:to-accent-500 active:from-accent-600 active:to-accent-700',
      destructive:
    ...
    size: {
      ...
      lg: 'text-base rounded-[12px] px-4 py-3',
...
export interface UnifiedButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'primary' | 'secondary' | 'accent' | 'destructive' | 'minimal' | 'neutral' | 'icon';
